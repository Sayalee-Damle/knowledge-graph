...
async def get_ontology_from_text(text: str):
    ...
    if not await chk_inp.check_if_text_exists(text):
        table = await ontology.return_ontology(text)
        ...
        await chk_inp.put_into_directory(text)
        db_text = await v_db.create_embeddings_text(text)
...
Further, the water is polluted in the name of industrial development, religious practices and more will cause a shortage of drinking water. Without water, human life is not possible. Moreover, the way waste is dumped on the land eventually ends up in the soil and turns toxic. If land pollution keeps on happening at this rate, we won’t have fertile soil to grow our crops on. Therefore, serious measures must be taken to reduce pollution to the core."""
    path_desc = asyncio.run(get_ontology_from_text(input_val_2))
